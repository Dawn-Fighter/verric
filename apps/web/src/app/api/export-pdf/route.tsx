...
import { getBranding, getDefaultBranding } from "@verric/storage";

export const runtime = "nodejs";

// Optional branding overrides applied at render time.
...
function isUnconfirmedFinding(finding: Finding) {
  const text = [
    ...
    ...finding.proofOfConcept.map((claim) => claim.text)
  ].join(" ");
  return (
    /potential|candidate|unconfirmed|not confirmed|requires further|no successful payload|needs poc/i.test(
      text
    ) ||
    finding.readiness === "needs_poc" ||
    finding.readiness === "unsupported"
  );
}
...
// NOTE: kept for future use. Currently superseded by collectUnverifiedClaims +
// inline filters above. Underscore-prefixed so ESLint allows the unused export-style helper.
function _partitionClaims(claims: ReportClaim[]): { verified: ReportClaim[]; unverified: ReportClaim[] } {
  return {
    ...
    rows.push({
      ...
      reason:
        ...
        (claim.status === "flagged"
          ? "Cited evidence does not support this claim."
          : "Cited evidence partially supports this claim.")
    });
  };
...
function findingEvidenceIds(finding: Finding) {
  return Array.from(
    new Set(
      [...finding.description, ...finding.impact, ...finding.proofOfConcept, ...finding.remediation].flatMap(
...
function Footer({ report, branding }: { report: VerricReport; branding?: BrandingPayload }) {
  const left = branding?.footerText || `${report.project.classification} · Penetration Test Report`;
  return (
...
function Cover({ report, branding }: { report: VerricReport; branding?: BrandingPayload }) {
  const navy = branding?.primaryColor || colors.navy;
  const subtitle = branding?.coverSubtitle || report.project.clientName;
  return (
    <Page size="A4" style={styles.cover}>
      <View style={{ ...styles.coverBar, backgroundColor: navy }} />
      {branding?.logoDataUrl ? (
        <Image src={branding.logoDataUrl} style={{ height: 48, marginBottom: 30 }} />
      ) : null}
      <Text style={{ ...styles.coverTitle, color: navy }}>Penetration Test Report</Text>
      <Text style={styles.coverSub}>{subtitle}</Text>
      <View style={{ ...styles.box, marginTop: 36 }}>
        <View style={styles.row}>
          <View style={styles.cell}>
            <Text style={styles.label}>Project</Text>
            <Text>{report.project.projectName}</Text>
          </View>
          <View style={styles.cell}>
            <Text style={styles.label}>Assessment Type</Text>
            <Text>{report.project.assessmentType}</Text>
          </View>
          <View style={styles.lastCell}>
            <Text style={styles.label}>Classification</Text>
            <Text>{report.project.classification}</Text>
          </View>
        </View>
        <View style={{ ...styles.row, borderTopWidth: 1, borderTopColor: colors.gray200 }}>
          <View style={styles.cell}>
            <Text style={styles.label}>Prepared By</Text>
            <Text>{report.project.preparedBy}</Text>
          </View>
          <View style={styles.cell}>
            <Text style={styles.label}>Lead Tester</Text>
            <Text>{report.project.testerName}</Text>
          </View>
          <View style={styles.lastCell}>
            <Text style={styles.label}>Report Date</Text>
            <Text>{report.project.reportDate}</Text>
          </View>
        </View>
      </View>
      <View style={{ marginTop: 34 }}>
        ...
        <Text
          style={{ fontSize: 26, color: severityColor(report.overallRisk), fontFamily: "Helvetica-Bold" }}
        >
          {report.overallRisk}
        </Text>
      </View>
...
function SummaryPage({
  ...
  const counts = ["Critical", "High", "Medium", "Low", "Informational"].map((severity) => ({
    severity,
    count: confirmed.filter((finding) => finding.severity === severity).length
  }));
  ...
  return (
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.section}>
        {report.executiveSummary.filter(isVerified).map((claim) => (
          <Text key={claim.id} style={{ marginBottom: 6 }}>
            {claim.text}
          </Text>
        ))}
      </View>
      <View style={styles.section}>
        ...
        <View style={styles.box}>
          <View style={styles.row}>
            <View style={styles.cell}>
              <Text style={styles.label}>Confirmed Findings</Text>
              <Text>{String(confirmed.length)}</Text>
            </View>
            <View style={styles.cell}>
              <Text style={styles.label}>Items Requiring Validation</Text>
              <Text>{String(unconfirmed.length)}</Text>
            </View>
            <View style={styles.lastCell}>
              <Text style={styles.label}>Overall Risk</Text>
              <Text style={{ color: severityColor(report.overallRisk), fontFamily: "Helvetica-Bold" }}>
                {report.overallRisk}
              </Text>
            </View>
          </View>
        ...
        <View style={styles.box}>
          <View style={styles.row}>
            {counts.map((item, index) => (
              <View key={item.severity} style={index === counts.length - 1 ? styles.lastCell : styles.cell}>
                <Text style={styles.label}>{item.severity}</Text>
                <Text
                  style={{ fontSize: 18, color: severityColor(item.severity), fontFamily: "Helvetica-Bold" }}
                >
                  {item.count}
                </Text>
              </View>
      ...
      <View style={styles.section}>
        ...
        {report.keyRecommendations.filter(isVerified).map((claim) => (
          <Text key={claim.id} style={{ marginBottom: 5 }}>
            • {claim.text}
          </Text>
        ))}
      </View>
        ...
        <View style={styles.box}>
          <View style={styles.specRow} wrap={false}>
            ...
            <Text style={styles.specValue}>
              {report.project.startDate} to {report.project.endDate}
            </Text>
          </View>
          <View style={styles.specRow} wrap={false}>
...
function RiskMethodologyPage({ report, branding }: { report: VerricReport; branding?: BrandingPayload }) {
  const rows = [
  ...
  return (
    <Page size="A4" style={styles.page}>
      <Footer report={report} branding={branding} />
      <View style={styles.rule}>
        <Text style={styles.h1}>Risk Rating Methodology</Text>
      </View>
      <Text style={{ marginBottom: 12 }}>
        Severity ratings are based on the technical characteristics of the issue, demonstrated exploitability,
        affected asset exposure, and likely business impact. CVSS is used as a baseline where sufficient
      ...
      <View style={styles.box}>
        {rows.map((row, _index) => (
          <View key={row[0]} style={styles.tableRow}>
            <Text
              style={{ ...styles.td, width: 90, color: severityColor(row[0]), fontFamily: "Helvetica-Bold" }}
            >
              {row[0]}
            </Text>
            <Text style={{ ...styles.td, width: 85 }}>{row[1]}</Text>
            <Text style={{ ...styles.td, flex: 1 }}>{row[2]}</Text>
          </View>
        ))}
      </View>
    </Page>
...
function FindingsSummaryPage({
  ...
  confirmed: Finding[];
  unconfirmed: Finding[];
  unverifiedClaims: UnverifiedRow[];
  branding?: BrandingPayload;
}) {
  return (
    <Page size="A4" style={styles.page}>
      <Footer report={report} branding={branding} />
      <View style={styles.rule}>
        <Text style={styles.h1}>Findings Summary</Text>
      </View>
      <View style={styles.box}>
        <View style={styles.tableRow}>
          <Text style={{ ...styles.th, width: 60 }}>ID</Text>
          <Text style={{ ...styles.th, flex: 1 }}>Finding</Text>
          <Text style={{ ...styles.th, width: 70 }}>Severity</Text>
          <Text style={{ ...styles.th, width: 60 }}>CVSS</Text>
          <Text style={{ ...styles.th, width: 130 }}>Affected Asset</Text>
        </View>
        {confirmed.map((finding) => (
          <View key={finding.id} style={styles.tableRow}>
            <Text style={{ ...styles.td, width: 60, color: colors.blue, fontFamily: "Helvetica-Bold" }}>
              {finding.id}
            </Text>
            <Text style={{ ...styles.td, flex: 1 }}>{finding.title}</Text>
            <Text
              style={{
                ...styles.td,
                width: 70,
            ...
            <Text style={{ ...styles.td, width: 60 }}>{finding.cvss}</Text>
            <Text style={{ ...styles.td, width: 130 }}>{finding.affectedAssets.join(", ") || "N/A"}</Text>
          </View>
        ))}
      </View>
      {unconfirmed.length > 0 ? (
        <View style={{ marginTop: 18 }}>
          ...
          <Text style={{ marginBottom: 8 }}>
            The following observations were not included as confirmed vulnerabilities because the supplied
            evidence does not prove exploitation.
          </Text>
          <View style={styles.box}>
            <View style={styles.tableRow}>
              <Text style={{ ...styles.th, width: 60 }}>ID</Text>
            ...
            {unconfirmed.map((finding) => (
              <View key={finding.id} style={styles.tableRow}>
                <Text
                  style={{ ...styles.td, width: 60, color: colors.gray700, fontFamily: "Helvetica-Bold" }}
                >
                  {finding.id}
                </Text>
                <Text style={{ ...styles.td, width: 200 }}>{finding.title}</Text>
                <Text style={{ ...styles.td, flex: 1 }}>
                  {finding.readinessSummary || "Additional PoC required"}
                </Text>
              </View>
            ))}
          </View>
      ...
      {unverifiedClaims.length > 0 ? (
        <View style={{ marginTop: 18 }} wrap={false}>
          ...
          <Text style={{ marginBottom: 8 }}>
            Verric&apos;s grounding pass could not independently confirm that the cited evidence supports the
            following statements. They have been removed from the polished body of this report and are listed
              ...
              <View key={`${row.source}-${idx}`} style={styles.tableRow}>
                ...
                <Text
                  style={{
                    ...styles.td,
                    width: 70,
                    color: row.status === "flagged" ? colors.red : colors.amber,
                    fontFamily: "Helvetica-Bold"
                  }}
                >
                  {row.status === "flagged" ? "Unsupported" : "Partial"}
                </Text>
        ...
        <View style={styles.box}>
          <View style={styles.row}>
            <View style={styles.cell}>
              ...
              {report.remediationRoadmap.immediate.map((item) => (
                <Text key={item}>• {item}</Text>
              ))}
            </View>
            <View style={styles.cell}>
              <Text style={styles.label}>Short Term</Text>
              {report.remediationRoadmap.shortTerm.map((item) => (
                <Text key={item}>• {item}</Text>
              ))}
            </View>
            <View style={styles.lastCell}>
              <Text style={styles.label}>Medium / Long Term</Text>
              {[...report.remediationRoadmap.mediumTerm, ...report.remediationRoadmap.longTerm].map(
                (item) => (
                  <Text key={item}>• {item}</Text>
                )
              )}
...
function SpecRow({
  ...
  return (
    <View style={last ? { ...styles.specRow, borderBottomWidth: 0 } : styles.specRow} wrap={false}>
      ...
      <Text style={mono ? { ...styles.specValue, fontFamily: "Courier", fontSize: 8 } : styles.specValue}>
        {value}
      </Text>
    </View>
  );
...
function FindingSection({ finding, chunks }: { finding: Finding; chunks: Map<string, EvidenceChunk> }) {
  const evidence = findingEvidenceIds(finding)
    .map((id) => chunks.get(id))
    .filter(Boolean)
    .slice(0, 8) as EvidenceChunk[];
  return (
    <View style={styles.finding} wrap={true}>
      <View style={styles.findingHead} wrap={false}>
        ...
        <Text style={styles.findingMeta}>
          ...
          <Text style={{ color: severityColor(finding.severity), fontFamily: "Helvetica-Bold" }}>
            {finding.severity}
          </Text>
          {"  ·  "} CVSS {finding.cvss}
        </Text>
      ...
      <View style={{ padding: 12 }}>
        ...
        {evidence.length > 0 ? (
          <View style={{ marginTop: 10 }} wrap={false}>
            ...
            {evidence.map((chunk) => (
              <Text key={chunk.id} style={styles.code}>
                {chunk.id} · {chunk.artifactName}:{chunk.lineStart} · {chunk.text}
              </Text>
            ))}
          </View>
        ) : null}
...
function EvidencePage({
  ...
  const usedChunks = usedIds
    .map((id) => map.get(id))
    .filter(Boolean)
    .slice(0, 45) as EvidenceChunk[];
  ...
  return (
    <Page size="A4" style={styles.page}>
      ...
      <Text style={{ marginBottom: 10 }}>
        The appendix includes cited evidence excerpts and uploaded screenshots supporting the confirmed
        findings.
      </Text>
      <View style={styles.box}>
        <View style={styles.tableRow}>
        ...
        {usedChunks.map((chunk) => (
          <View key={chunk.id} style={styles.tableRow} wrap={false}>
            ...
            <Text style={{ ...styles.td, width: 155 }}>
      ...
      <View style={{ marginTop: 18 }} wrap={false}>
        ...
        <Text>
          This report is based on a point-in-time review of the supplied evidence and agreed scope. It does
          not guarantee that all possible vulnerabilities were identified. Findings should be validated by the
          testing team and remediated according to organizational risk tolerance.
        </Text>
      </View>
    </Page>
  );
...
function DetailedFindingsPage({
  ...
  return (
    <Page size="A4" style={styles.page}>
      ...
      {confirmed.map((finding) => (
        <FindingSection key={finding.id} finding={finding} chunks={chunks} />
      ))}
    </Page>
  );
}
...
function ReportPdf({
  ...
  return (
    <Document title={`${report.project.clientName} Penetration Test Report`}>
      <Cover report={report} branding={branding} />
      <SummaryPage report={report} confirmed={confirmed} unconfirmed={unconfirmed} branding={branding} />
      <RiskMethodologyPage report={report} branding={branding} />
      <FindingsSummaryPage
        report={report}
        confirmed={confirmed}
        ...
        branding={branding}
      />
      <DetailedFindingsPage report={report} confirmed={confirmed} chunks={map} branding={branding} />
      <EvidencePage report={report} chunks={chunks} artifacts={artifacts} branding={branding} />
    </Document>
  );
}
...
export async function POST(request: Request) {
  ...
  const { report, chunks, artifacts } = body;

  // Resolve branding: explicit payload > brandingId from DB > default branding from DB > none.
  let branding: BrandingPayload | undefined = body.branding;
  if (!branding) {
    try {
      const db = getDb();
      const row = body.brandingId ? getBranding(db, body.brandingId) : getDefaultBranding(db);
      if (row) {
        branding = {
          ...
          coverSubtitle: row.coverSubtitle
        };
      }
    } catch {
      // DB unavailable — fall back to no branding.
    }
  }

  const pdf = await renderToBuffer(
    <ReportPdf report={report} chunks={chunks || []} artifacts={artifacts || []} branding={branding} />
  );
  return new NextResponse(pdf, {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="verric-${report.project.clientName || "client"}-report.pdf"`
    }
  });
}
