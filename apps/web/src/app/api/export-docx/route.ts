...
const p = (
  value: string,
  options: { bold?: boolean; color?: string; size?: number; spacing?: number } = {}
...
const label = (value: string) =>
  new Paragraph({
    children: [
      new TextRun({ text: value.toUpperCase(), bold: true, color: MUTED, size: 16, font: "Courier New" })
    ],
    spacing: { after: 60 }
  });
...
function specCell(
  value: string,
  options: { bold?: boolean; color?: string; mono?: boolean; header?: boolean } = {}
) {
...
function claimParagraphs(title: string, claims: ReportClaim[]) {
  ...
  return [
    label(title),
    ...verified.map((claim) =>
      p(`${claim.text}${claim.evidenceIds.length ? ` [${claim.evidenceIds.join(", ")}]` : ""}`)
    )
  ];
}
...
function unverifiedClaimsTable(rows: UnverifiedRow[]) {
  return new Table({
    ...
    rows: [
      new TableRow({
        children: ["Source", "Statement", "Status", "Reviewer Note"].map(
          (heading) =>
            new TableCell({
              ...
              children: [
                new Paragraph({
                  children: [new TextRun({ text: heading, bold: true, color: MUTED, size: 18 })],
                  spacing: { after: 0 }
                })
              ]
            })
          ...
          new TableRow({
            children: [
              new TableCell({
                ...
                children: [
                  new Paragraph({
                    children: [new TextRun({ text: row.source, color: MUTED, size: 20 })],
                    spacing: { after: 0 }
                  })
                ]
              }),
              new TableCell({
                ...
                children: [
                  new Paragraph({
                    children: [new TextRun({ text: row.text, color: INK, size: 20 })],
                    spacing: { after: 0 }
                  })
                ]
              }),
              new TableCell({
                ...
                children: [
                  new Paragraph({
                    children: [
                      new TextRun({
                        text: row.status === "flagged" ? "Unsupported" : "Partial",
                        bold: true,
                        color: row.status === "flagged" ? RED : "A15C07",
                        size: 20
                      })
                    ],
                    spacing: { after: 0 }
              ...
              new TableCell({
                ...
                children: [
                  new Paragraph({
                    children: [new TextRun({ text: row.reason, color: INK, size: 20 })],
                    spacing: { after: 0 }
                  })
                ]
              })
...
function findingSection(finding: Finding, chunkMap: Map<string, EvidenceChunk>) {
  ...
  return [
    ...
    ...(evidence.length
      ? [
          label("Key Evidence Excerpts"),
          ...evidence.map((chunk) =>
            p(`${chunk.id} | ${chunk.artifactName}:${chunk.lineStart} | ${chunk.text}`)
          )
        ]
      : []),
...
export async function POST(request: Request) {
  ...
  const usedEvidence = Array.from(new Set(confirmedFindings.flatMap(findingEvidenceIds)))
    .map((id) => chunkMap.get(id))
    .filter(Boolean)
    .slice(0, 50) as EvidenceChunk[];

  const findingsRows = [
    new TableRow({
      children: ["ID", "Finding", "Severity", "CVSS", "Affected Asset"].map((item) =>
        cell([p(item, { bold: true, color: MUTED })])
      )
    }),
    ...confirmedFindings.map(
      (finding) =>
        new TableRow({
          children: [
            finding.id,
            ...
            finding.severity,
            finding.cvss,
            finding.affectedAssets.join(", ") || "N/A"
          ].map((item) => cell([p(item)]))
        })
    )
  ];
  ...
  const riskRows = [
    new TableRow({
      children: ["Rating", "CVSS", "Remediation Guidance"].map((item) =>
        cell([p(item, { bold: true, color: MUTED })])
      )
    }),
    ...[
      ...
      paragraphStyles: [
        {
          ...
          next: "Normal",
          quickFormat: true,
          run: { bold: true, size: 36, color: INK }
        },
        {
          id: "Heading2",
          name: "Heading 2",
          ...
          next: "Normal",
          quickFormat: true,
          run: { bold: true, size: 28, color: INK }
        }
      ]
    },
    sections: [
      {
        ...
        children: [
          new Paragraph({
            ...
            spacing: { after: 180 },
            children: [new TextRun({ text: "Penetration Test Report", bold: true, size: 44, color: INK })]
          }),
          new Paragraph({
            alignment: AlignmentType.CENTER,
            spacing: { after: 520 },
            children: [new TextRun({ text: report.project.clientName, bold: true, size: 40, color: RED })]
          }),
          table([
            new TableRow({
              children: [
                cell([label("Project"), p(report.project.projectName)]),
                cell([label("Assessment"), p(report.project.assessmentType)]),
                cell([label("Classification"), p(report.project.classification)])
              ]
            }),
            new TableRow({
              children: [
                cell([label("Prepared By"), p(report.project.preparedBy)]),
                cell([label("Lead Tester"), p(report.project.testerName)]),
                cell([label("Report Date"), p(report.project.reportDate)])
              ]
            })
          ]),
          h("Document Control"),
          ...
          specTable([
            ...
            ...(report.project.rulesOfEngagement
              ? [["Rules of Engagement", report.project.rulesOfEngagement] as [string, string]]
              : []),
            ...(report.project.toolsUsed
              ? [["Tools Used", report.project.toolsUsed] as [string, string]]
              : [])
          ]),
          label("Methodology"),
          ...
          ...(unconfirmedFindings.length > 0
            ? [
                ...
                p(
                  "The following observations are not included as confirmed vulnerabilities because the supplied evidence does not prove exploitation."
                ),
                ...unconfirmedFindings.map((finding) =>
                  p(
                    `${finding.id}: ${finding.title} - ${finding.readinessSummary || "Additional PoC required."}`
                  )
                )
              ]
            : []),
          ...(unverifiedClaims.length > 0
            ? [
                ...
                p(
                  "Verric's grounding pass could not independently confirm that the cited evidence supports the following statements. They have been removed from the polished body of this report and are listed here for reviewer attention."
                ),
          ...
          ...[...report.remediationRoadmap.mediumTerm, ...report.remediationRoadmap.longTerm].map((item) =>
            bullet(item)
          ),
          h("Evidence Reference Index"),
          p(
            "The following evidence references were cited by the report findings. Raw tool output is summarized rather than dumped in full."
          ),
          ...usedEvidence.map((chunk) =>
            p(`${chunk.id} | ${chunk.artifactName}:${chunk.lineStart} | ${chunk.text}`)
          ),
          ...imageParagraphs(artifacts || []),
          h("Disclaimer & Limitations"),
          p(
            "This report is based on a point-in-time review of the supplied evidence and project scope. It does not guarantee that all possible vulnerabilities were identified. Findings should be validated by the testing team and remediated according to organizational risk tolerance."
          )
        ]
      }
    ]
